...
class BDRAR(nn.Module):
    ...
    def forward(self, x):
        ...
        # predict4_hl = F.sigmoid(F.upsample(predict4_hl, size=x.size()[2:], mode='bilinear'))
        # predict3_hl = F.sigmoid(F.upsample(predict3_hl, size=x.size()[2:], mode='bilinear'))
        # predict2_hl = F.sigmoid(F.upsample(predict2_hl, size=x.size()[2:], mode='bilinear'))
        # predict1_hl = F.sigmoid(F.upsample(predict1_hl, size=x.size()[2:], mode='bilinear'))
        # predict1_lh = F.sigmoid(F.upsample(predict1_lh, size=x.size()[2:], mode='bilinear'))
        # predict2_lh = F.sigmoid(F.upsample(predict2_lh, size=x.size()[2:], mode='bilinear'))
        # predict3_lh = F.sigmoid(F.upsample(predict3_lh, size=x.size()[2:], mode='bilinear'))
        # predict4_lh = F.sigmoid(F.upsample(predict4_lh, size=x.size()[2:], mode='bilinear'))
        # fuse_predict = F.sigmoid(F.upsample(fuse_predict, size=x.size()[2:], mode='bilinear'))
